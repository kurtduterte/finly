...
class ExpensesRepository {
  ...
  Future<void> addExpense(ExpensesCompanion entry) => db.transaction(() async {
    ...
    final account = await db.accountsDao.getById(entry.accountId.value);
    await db.accountsDao.updateAccount(
      account.copyWith(
        balanceCentavos: account.balanceCentavos - entry.amountCentavos.value,
      ),
    );
  });

  Future<void> updateExpense(Expense newExpense) => db.transaction(() async {
    final old = await db.expensesDao.getExpenseById(newExpense.id);

    final oldAccount = await db.accountsDao.getById(old.accountId);
    await db.accountsDao.updateAccount(
      oldAccount.copyWith(
        balanceCentavos: oldAccount.balanceCentavos + old.amountCentavos,
      ),
    );

    final newAccount = await db.accountsDao.getById(newExpense.accountId);
    await db.accountsDao.updateAccount(
      newAccount.copyWith(
        balanceCentavos: newAccount.balanceCentavos - newExpense.amountCentavos,
      ),
    );
  ...
  Future<void> deleteExpense(int id) => db.transaction(() async {
    ...
    final account = await db.accountsDao.getById(expense.accountId);
    await db.accountsDao.updateAccount(
      account.copyWith(
        balanceCentavos: account.balanceCentavos + expense.amountCentavos,
      ),
    );
  });
}
